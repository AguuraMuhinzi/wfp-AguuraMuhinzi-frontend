...
// export default AcademyPricePredictionDashboard;
  ...
  const AcademyPricePredictionDashboard = () => {
    ...
    // Redux selectors
    const { prediction: reduxPrediction, loading: predictionLoading } = useSelector(state => state.prediction);
  ...
  console.log('User input price:', userInputPrice);
                ...
                <div className="space-y-4">
                  {history.map((item, index) => (
                    <div key={index} className="border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 className="text-lg font-semibold text-gray-800 mb-1">{item.commodity}</h4>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm text-gray-600">
                            <div><span className="font-medium">District:</span> {item.district}</div>
                            <div><span className="font-medium">Period:</span> {months[item.month - 1]} {item.year}</div>
                            <div><span className="font-medium">Predicted:</span> {item.predicted_price.toLocaleString()} RWF</div>
                            <div><span className="font-medium">Range:</span> {item.lower_bound.toLocaleString()} – {item.upper_bound.toLocaleString()} RWF</div>
                            <div><span className="font-medium">Price Type:</span> {item.pricetype}</div>
