// src/pages/AcademyNotifications.jsx
...
const AcademyNotifications = () => {
  const dispatch = useDispatch();
  const userId = typeof window !== 'undefined' ? localStorage.getItem('user_id') : null;

  // Safe fallback
  const { list = [], isLoading = false, error = null } = useSelector((s) => s.notifications || {});
                ...
                <li key={n.id} className={`p-4 sm:p-5 ${!n.is_read ? 'bg-green-50/60' : 'bg-white'}`}>
                  ...
                  {expanded && (
                    <div className="mt-3 pl-7">
                      ...
                      {/* If you have deep links (e.g., to a course/lesson), add them here based on n.notification_type */}
                    </div>
                  )}
                </li>
              );
...
export default AcademyNotifications;