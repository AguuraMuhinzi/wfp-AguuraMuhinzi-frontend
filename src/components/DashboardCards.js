...
//       const uniqueAcademies = new Set(orders.map((order) => order.user.academy_details?.name));
...
const DashboardCards = () => {
  ...
  useEffect(() => {
    if (orders?.length) {
      // Filter orders associated with the logged-in user
      const userOrders = orders.filter((order) =>
        order.products.some((product) => product.user_id === parseInt(userId, 10))
      );

      // Calculate unique academies (active contracts)
      const uniqueAcademies = new Set(userOrders.map((order) => order.user.academy_details?.name));
      setContracts(uniqueAcademies.size);

      // Calculate pending and completed orders
      setPendingOrders(userOrders.filter((order) => order.status === 'pending').length);
      setCompletedOrders(userOrders.filter((order) => order.status === 'completed').length);
    }
  }, [orders, userId]);
