...
import { useDispatch, useSelector } from 'react-redux';
import { fetchCooperativeDetails } from '../Redux/Slices/cooperative/coop_details';
import { fetchNotifications } from '../Redux/Slices/notifications/notification_slice'; // <-- add this
import {
  FiHome, FiBox, FiClipboard, FiBarChart2,
  FiTrendingUp, FiUser, FiMessageCircle, FiBell, FiUser as FiUserIcon
} from 'react-icons/fi';
...
const DashboardLayout = () => {
  ...
  // Get cooperative user id from localStorage
  const userId = typeof window !== 'undefined' ? localStorage.getItem('user_id') : null;

  // Notifications slice (safe fallback)
  ...
  useEffect(() => {
    if (userId) {
      dispatch(fetchCooperativeDetails(userId));
      dispatch(fetchNotifications(userId)); // prefetch notifications
    }
  }, [dispatch, userId]);

  // Optional: Poll every 20s for fresh notifications
  useEffect(() => {
    if (!userId) return;
      ...
      <div className="flex flex-col flex-1 lg:pl-[260px] overflow-y-auto">
        ...
        <header className="sticky top-0 z-50 flex items-center justify-between bg-white border-b dark:bg-neutral-800 dark:border-neutral-700 px-4 py-2">
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Notification bell: shows unread badge, navigates to notifications page */}
            <Link
              to="/dashboard/copNotification"
              className="relative text-gray-500 dark:text-white hover:text-green-600"
              title="Notifications"
              onClick={() => { if (userId) dispatch(fetchNotifications(userId)); }}
            >
              <FiBell className="w-6 h-6" />
              {unreadCount > 0 && (
